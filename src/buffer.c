...
void buffer_line_apply_syntax_highlighting(Buffer *b, BufferLine *line, uint32_t start_byte, Theme *theme) {
    ...
    if (!b->cursor) {
        buffer_line_ensure_run_capacity(line, 1);
        line->runs[0].style = (CharStyle) {
            .r = theme->syntax_variable.fg_r,
            .g = theme->syntax_variable.fg_g,
            .b = theme->syntax_variable.fg_b,
            .style = theme->syntax_variable.style
        };
        line->runs[0].length = line->char_count;
        line->run_count = 1;
        return;
    }
    ...
    for (int char_idx = 0; char_idx < line->char_count; char_idx++) {
        ...
        if (line->run_count > 0 && memcmp(&line->runs[line->run_count - 1].style, &new_style, sizeof(CharStyle)) == 0) {
            line->runs[line->run_count - 1].length++;
        } else {
            buffer_line_ensure_run_capacity(line, line->run_count + 1);
            StyleRun *new_run = &line->runs[line->run_count];
            new_run->style = new_style;
            new_run->length = 1;
            line->run_count++;
        }
        current_byte += strlen(line->chars[char_idx].value);
    }
...
void buffer_line_realloc_for_capacity(BufferLine *line, int new_needed_capacity) {
    ...
    line->chars = realloc(line->chars, new_capacity * sizeof(Char));
    if (line->chars == NULL) {
...
void buffer_line_ensure_run_capacity(BufferLine *line, int new_run_capacity) {
    if (new_run_capacity > line->run_capacity) {
        int old_capacity = line->run_capacity;
        line->run_capacity = new_run_capacity > old_capacity * 2 ? new_run_capacity : old_capacity * 2;
        if (line->run_capacity == 0) line->run_capacity = 4;
        line->runs = realloc(line->runs, sizeof(StyleRun) * line->run_capacity);
        if (!line->runs) {
            log_error("buffer.buffer_line_ensure_run_capacity: realloc runs failed");
            exit(1);
        }
    }
}
...
void buffer_line_init(BufferLine *line) {
    ...
    if (line->chars == NULL) {
        log_error("buffer.buffer_line_init: failed to allocate BufferLine chars");
        exit(1);
    }
    line->run_count = 0;
    line->run_capacity = 0;
